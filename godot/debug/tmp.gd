...
func _ready():
	...
	print_debug("Entity is ", $RigidBody2D is RigidBody2D)
	print_debug("The real Entity is ", $RigidBody2D is Entity)
	print_debug(global.get_base_entity($Sprite))
	print_debug(global.get_base_entity($RigidBody2D))
	...
	if Input.get_connected_joypads().size() > 0:
		 for i in range(16):
	      if Input.is_joy_button_pressed(0,i):
	        print_debug("Button at " + str(i) + " pressed, should be button: " + Input.get_joy_button_string(i))
