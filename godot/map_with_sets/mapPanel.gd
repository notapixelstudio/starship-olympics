...
func set_chosen(chosen_):
	...
	if chosen:
		...
		$Tween.interpolate_property(self, 'position',
			position, Vector2(position.x,-30), 0.5,
			Tween.TRANS_CUBIC, Tween.EASE_OUT)
		$Tween.start()
	else:
		$Background.self_modulate = deselected_modulate
		$Tween.interpolate_property(self, 'position',
			position, Vector2(position.x,0), 0.5,
			Tween.TRANS_CUBIC, Tween.EASE_OUT)
		$Tween.start()
...
func set_player(v : InfoPlayer):
	assert(v != null)
	player = v
	$Sprite2D.texture = player.species.ship
	$Info.text = rest_text
...
func create_minicards():
	...
	for minigame in content.minigames:
		var minicard = Minicard.instantiate()
