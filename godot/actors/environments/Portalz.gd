...
@export var thickness : float = 10: set = set_thickness
@export var width : float = 100: set = set_width
@export var offset : float = 30: set = set_offset
...
func refresh():
	...
	$Area2D/CollisionShape2D.shape.size = Vector2(width, thickness)
	$StaticBody2D/CollisionShape2D.shape.size = Vector2(width, thickness)
