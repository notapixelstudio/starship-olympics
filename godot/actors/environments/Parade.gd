...
func spawn():
	...
	for col in range(min_col, max_col+1):
		if get_cell(col, row) == ALIEN:
			...
			alien.global_position = to_global(map_to_local(Vector2(col, row))) + Vector2(100, 100)
			...
			mine.global_position = to_global(map_to_local(Vector2(col, row))) + Vector2(100, 100)
