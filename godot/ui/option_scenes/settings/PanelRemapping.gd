...
func _ready():
	...
	Events.connect("remap_done", self, "update_mapping")
...
func show_remap_scene(action: String, remap_action_asker: RemapAction):
	var remap: RemappingScene = what_to_remap_scene.instance()
	remap.action = action
	remap.device_category = self.device_category
	$CanvasLayer.add_child(remap)
	remap.connect("remap_event", remap_action_asker, "on_remap")
