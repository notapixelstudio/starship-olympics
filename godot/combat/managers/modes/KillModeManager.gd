...
func _on_sth_killed(sth, killer : Ship, ship_for_good=false):
	...
	if sth is Ship:
		...
		if not killer or killer is Ship and killer.get_team() == sth.get_team():
			...
			emit_signal('show_msg', sth.get_color(), -score_multiplier, sth.position)
		...
		if killer and killer is Ship and killer.get_team() != sth.get_team():
			...
			emit_signal('show_msg', killer.get_color(), score_multiplier, sth.position)
		...
		emit_signal('show_msg', killer.get_color(), score, sth.position)
