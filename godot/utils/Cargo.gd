...
func drop_holdable(cause):
	...
	just_dropped = true
	await get_tree().create_timer(0.5).timeout # maybe we should use a Timer node, to enable canceling
	just_dropped = false
	
func show_holdable():
	if held != null:
		$Wrapper/Sprite2D.texture = held.get_texture()
		$Wrapper/Shadow.texture = held.get_texture()
		if held.show_on_top():
			$Wrapper/Sprite2D.z_index = 100
			$Wrapper/Sprite2D.z_as_relative = false
		...
		$Wrapper/Sprite2D.scale = Vector2(1.3, 1.3) if held is Alien else Vector2(1,1)
		$Wrapper/Shadow.scale = Vector2(1.3, 1.3) if held is Alien else Vector2(1,1)

func hide_holdable():
	$Wrapper/Sprite2D.texture = null
	$Wrapper.position = Vector2(0,0)
	$Wrapper/Sprite2D.z_index = 0
	$Wrapper/Sprite2D.z_as_relative = true
	$RoyalGlow.visible = false

func _process(delta):
	if held != null and not held.is_rotatable():
		$Wrapper/Sprite2D.rotation = -global_rotation
