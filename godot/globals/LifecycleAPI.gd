...
func _init():
	...
	Events.connect('draft_ended', Callable(self, '_on_draft_ended'))
	Events.connect('analytics_enabled', Callable(self, '_on_analytics_enabled'))
	Events.connect('analytics_disabled', Callable(self, '_on_analytics_disabled'))
...
func _on_game_ended() -> void:
	...
	var data = {"id":global.the_game.get_uuid(),"duration_ms":Time.get_ticks_msec() - game_started_ms}
	Events.emit_signal("analytics_event", data, event_name)
...
func _on_session_ended() -> void:
	...
	var data = {"id":global.session.get_uuid(),"duration_ms":Time.get_ticks_msec() - session_started_ms}
	Events.emit_signal("analytics_event", data, event_name)
...
func _on_match_ended(match_dict: Dictionary) -> void:
	...
	var data = {"id":global.the_match.get_uuid(),"duration_ms":Time.get_ticks_msec() - game_started_ms, "minigame_id": global.the_match.get_minigame_id(), "card_id":  global.the_match.get_card_id()}
	Events.emit_signal("analytics_event", data, event_name)
