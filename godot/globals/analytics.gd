...
func enable():
	Events.connect("analytics_event", Callable(self, '_on_analytics_event'))
	Events.emit_signal("analytics_enabled")

func disable():
	if Events.is_connected("analytics_event", Callable(self, '_on_analytics_event')):
		Events.emit_signal("analytics_disabled")
		Events.disconnect("analytics_event", Callable(self, '_on_analytics_event'))
...
func send_event(event_body: Dictionary, event_name: String):
	...
	http_request.connect("request_completed", Callable(self, "_http_request_completed").bind(http_request, event_body))
