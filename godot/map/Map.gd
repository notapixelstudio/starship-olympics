...
export (String, 'map_locked', 'normal', 'win_stage') var state setget change_state
...
export var inside_players: Array
...
func normal():
	...
	camera.enabled = true
	var players = self.create_standalone_players()
	if global.session:
...
func win_stage():
	...
	if global.session:
		winner = global.session.get_winner()
	else:
		var standalone_players : Dictionary = self.create_standalone_players()
		winner = standalone_players["p1"]
	assert (winner is InfoPlayer)
	self.set_cursors_in_map({winner.id: winner})


func change_state(value: String):
	if not value:
		return
...
func create_standalone_players() -> Dictionary:
	var players = {}
	var i = 1
	var already_selected_species = []
	for controls_player in inside_players:
		var player: InfoPlayer = InfoPlayer.new()
		player = player.random_species(already_selected_species)
		player.id = "p" + str(i)
		player.controls = controls_player
		already_selected_species.append(player.species)
